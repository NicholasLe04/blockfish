...
def w_pawn_double_push_targets(w_pawn_bb, all_pieces_bb):
    ...
    return (single_push << 8) & empty_bb & constants.RANK_4


# White Captures
def _w_pawn_east_attacks(w_pawn_bb):
    return ((w_pawn_bb << 7) & constants.EXCLUDE_A_FILE)

def _w_pawn_west_attacks(w_pawn_bb):
    return ((w_pawn_bb << 9) & constants.EXCLUDE_H_FILE)
...
def b_pawn_double_push_targets(b_pawn_bb, all_pieces_bb):
    ...
    return (single_push >> 8) & empty_bb & constants.RANK_5


# Black Captures
def _b_pawn_east_attacks(b_pawn_bb):
    return ((b_pawn_bb >> 9) & constants.EXCLUDE_A_FILE)

def _b_pawn_west_attacks(b_pawn_bb):
    return ((b_pawn_bb >> 7) & constants.EXCLUDE_H_FILE)
